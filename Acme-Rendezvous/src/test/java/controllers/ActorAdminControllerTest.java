...
public class ActorAdminControllerTest extends AbstractTest {
	...
	public void testNoAuthenticatedCanRegisterAsAdmin() throws Exception {
		...
		this.mockMvc
			.perform(
				MockMvcRequestBuilders.post("/actor/admin/register.do").contentType(MediaType.APPLICATION_FORM_URLENCODED).param("name", "Fernando").param("surname", "Gutiï¿½rrez Lï¿½pez").param("birthDate", "09/04/2000")
					.param("email", "ferguti90@gmail.com").param("phoneNumber", "606587789").param("postalAddress", "Calle Picadero 9").param("userAccount.username", "fernanguti").param("userAccount.password", "fernanguti")
					.param("confirmPassword", "fernanguti").sessionAttr("user", new User()).param("save", "")).andExpect(MockMvcResultMatchers.status().is(302)).andExpect(MockMvcResultMatchers.view().name("redirect:/welcome/index.do"))
